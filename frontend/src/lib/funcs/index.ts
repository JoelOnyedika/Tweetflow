...
export const getCsrfToken = async () => {
  const response = await fetch(
    `${import.meta.env.VITE_BACKEND_SERVER_URL}/api/csrf/`,
    {
      method: "GET",
      credentials: "include",
    }
...
export const chopUserCredits = async (userId, amountOfCredits) => {
  try {
    ...
    const getCreditsResponse = await fetch(
    ...
    if (getCreditsData.data) {
      ...
      if (getCreditsData.data[0].credits > amountOfCredits) {
        const chopResponse = await fetch(
          `${
            import.meta.env.VITE_BACKEND_SERVER_URL
          }/api/chop-credits/${userId}/`,
          {
            method: "POST",
            credentials: "include",
            headers: {
        ...
        if (!chopResponse.ok || chopResponseData.error) {
          ...
          return {
            error: chopResponseData.error || {
              message: "Error chopping credits",
            },
          };
        }
    ...
    console.error(error);
    return {
      error: { message: "Something went wrong while handling credits" },
    };
  }
};
