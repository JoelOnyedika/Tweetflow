  ...
  useEffect(() => {
    const fetchVoiceModels = async () => {
      ...
      try {
        ...
        if (data) {
          // Assume that `data` contains an array of voice models
          console.log(data)
          // const modelsWithPlanInfo = data.map((model) => ({
          //   ...model,
          //   inPlan: plan === 'Enterprise' || (plan === 'premium' && model.plan === 'premium'), 
          //   // Include logic to check if the model is in the user's plan
          // }));

          setVoiceModels(data); // Set voice models with `inPlan` field
        } else {
