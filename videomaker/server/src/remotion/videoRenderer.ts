...
interface RenderVideoParams {
  videoId: string;
  data: Record<string, any>; // Replace this with the exact type of your inputProps
}

export const renderVideo = async ({ videoId, data }: RenderVideoParams): Promise<string> => {
  try {
    ...
    const bundleLocation = await bundle(
      ...
      undefined,
      {
        webpackOverride: (config: any) => config, // If you have webpack customizations
      }
    );
